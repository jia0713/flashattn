...
inline __device__ void compute_dq_dk_dv_1colblock_hdim64_64x64_4waves(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    Tensor tdStsdSt = make_tensor(make_smem_ptr(dStsmem_ptr_sts), make_layout(Shape<_4, _1, _4>{},
                                                                          Stride<_1, _0, Int<16*64>>{}));
    Element *Ptsmem_ptr_sts = reinterpret_cast<Element *>(sP.data().get()) + warp_id * 4 * 64 + row * 64 + 4 * col;
    Tensor tPtsPt = make_tensor(make_smem_ptr(Ptsmem_ptr_sts), make_layout(Shape<_4, _1, _4>{},
                                                                          Stride<_1, _0, Int<16*64>>{}));
    ...
    Element *dStsmem_ptr_lds = reinterpret_cast<Element *>(sdS.data().get()) + lds_row * 64 + lds_col * 4;
    Tensor tdKsdSt = make_tensor(make_smem_ptr(dStsmem_ptr_lds), make_layout(Shape<_4, _4, _4>{},
                                                                          Stride<_1, Int<16*64>, Int<4*64>>{}));
    Element *Ptsmem_ptr_lds = reinterpret_cast<Element *>(sP.data().get()) + lds_row * 64 + lds_col * 4;
    Tensor tdVsPt = make_tensor(make_smem_ptr(Ptsmem_ptr_lds), make_layout(Shape<_4, _4, _4>{},
                                                                          Stride<_1, Int<16*64>, Int<4*64>>{}));
    ...
    auto ldsQ_coord = thread_layout_Q.get_hier_coord(threadIdx.x);
    ...
    Tensor tKtrKt = make_tensor(tKrK.data(), make_layout(Shape<_4, _4, _1>{}));
    permute_4x4_b16(tKtrKt);

    auto skt_sts_row = threadIdx.x / 4 ;
    auto skt_sts_col = threadIdx.x % 4;
    auto skt_sts_ptr = sKt.data().get() + skt_sts_row * 64 + skt_sts_col * 16;

    *(reinterpret_cast<cute::uint128_t*>(skt_sts_ptr)) = *(reinterpret_cast<cute::uint128_t*>(tKtrKt.data()));
    *(reinterpret_cast<cute::uint128_t*>(skt_sts_ptr + 8)) = *(reinterpret_cast<cute::uint128_t*>(tKtrKt.data() + 8));
    ...
    auto skt_lds_ptr = sKt.data().get() + skt_lds_row * 4 * 64 + skt_lds_col * 4;
    auto tdQsKt  = make_tensor(make_smem_ptr(skt_lds_ptr), make_layout(Shape<_4,_4,_4>{}, Stride<_1,_64,_1024>{}));
    ...
    for (; m_block >= m_block_min; --m_block) {
        ...
        cute::copy(tdSrdS, tdQrdS);
        tdQgdQaccum.data() = tdQgdQaccum.data() + (-dq_accum_block_stride);
        clear(acc_dq);
        flash::gemm</*A_in_regs=*/true, /*B_in_regs=*/Kernel_traits::Is_K_in_regs>(acc_dq, tdQrdS, tdQrKt, tdQsdS, tdQsKt, tiled_mma_dq,
        ...
        gemm_transB(acc_dk,tdKrdSt,tdKrQt,tdKsdSt,tdKsQt,tiled_mma_dkv);

        gemm_transB(acc_dv,tdVrPt,tdVrdOt,tdVsPt,tdVsdOt,tiled_mma_dkv);
    }
