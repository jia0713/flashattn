...
__forceinline__ __device__ void compute_dq_dk_dv_1colblock_k64_xcore1500(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    Tensor sV = make_tensor(sK.data() + size(sK), typename Kernel_traits::VarHeadDim::SmemLayoutV_swz333{});
    Tensor sV_NoSwizzle = make_tensor(sV.data(), typename Kernel_traits::VarHeadDim::SmemLayoutV_NoSwizzle{});

    //reuse Kernel_traits::Is_V_in_regs ?
    ...
    Tensor sdO = make_tensor(sQ.data() + (Double_buffer ? 2 : 1)*size(sQ), typename Kernel_traits::VarHeadDim::SmemLayoutdO_swz242{});
    Tensor sdO_NoSwizzle = make_tensor(sdO.data(), typename Kernel_traits::VarHeadDim::SmemLayoutdO_NoSwizzle{});
    Tensor sdOt = make_tensor(sdO.data(), typename Kernel_traits::VarHeadDim::SmemLayoutdOtransposed_swz242{});
    Tensor sdOt_NoSwizzle = make_tensor(sdO.data(), typename Kernel_traits::VarHeadDim::SmemLayoutdOtransposedNoSwizzle{});
    ...
    Tensor tdOcdO_noSwizzle = gmem_thr_copy_ldgbsm.partition_S(cdO);       // (ACPY,ACPY_M,ACPY_K) -> (blk_m,blk_k)
    Tensor tKcK_noSwizzle = gmem_thr_copy_ldgbsm.partition_S(cK);       // (BCPY,BCPY_N,BCPY_K) -> (blk_n,blk_k)
    Tensor tVcV_noSwizzle = gmem_thr_copy_ldgbsm.partition_S(cV);        // (BCPY,BCPY_N,BCPY_K) -> (blk_n,blk_k)
    Tensor tQcQ = make_tensor(tQcQ_noSwizzle.data() + make_coord(0, swz242_offset + swz242_diff_global), layout(tQcQ_noSwizzle));
    Tensor tdOcdO = make_tensor(tdOcdO_noSwizzle.data() + make_coord(0, swz242_offset + swz242_diff_global), layout(tdOcdO_noSwizzle));
    Tensor tKcK = make_tensor(tKcK_noSwizzle.data() + make_coord(0, swz242_offset), layout(tKcK_noSwizzle));
    Tensor tVcV = make_tensor(tVcV_noSwizzle.data() + make_coord(0, swz333_offset), layout(tVcV_noSwizzle));
    ...
    flash::copy_b128_bsm_async<Is_even_MN, Is_even_K>(
        tdOgdO, tdOsdO, tdOcdO, params.d, binfo.actual_seqlen_q - m_block * kBlockM
    );
    ...
    auto process_m_block = [&](auto Need_mask) {
        ...
        if (m_block > m_block_min) {
            ...
            // Double buffer
            tdOsdO.data() = tdOsdO.data() + sdO_offset;
            // Prefetch global -> smem : dO
            tdOgdO.data() = tdOgdO.data() + (-int(kBlockM * params.do_row_stride));
            flash::copy_b128_bsm_async</*Is_even_MN=*/true, Is_even_K>(tdOgdO, tdOsdO, tdOcdO, params.d);
        }
        ...
        if constexpr (Is_dropout) {
            ...
            // we're doing dropout with block of 16 x 64
            int block_col_idx = kBlockN <= 32 ? n_block / (64 / kBlockN) : n_block * (kBlockN / 64);
        ...
        tdKsQt.data() = tdKsQt.data() + sQ_offset;
        ...
        cute::copy(smem_tiled_copy_dS, tdQsdS, tdQrdS);   // lds
        // Double buffer
        tdPsdO.data() = tdPsdO.data() + sdO_offset;
        tdVsdOt.data() = tdVsdOt.data() + sdO_offset;
        ping_pong ^= 1;

        clear(acc_dq);
        // lds, reg
        flash::gemm</*A_in_regs=*/true, /*B_in_regs=*/true>(
