...
inline __device__ void compute_dq_dk_dv_1colblock_hdim96_16x64_2waves(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    Tensor sQ = make_tensor(sV.data(), typename Kernel_traits::SmemLayoutQdO{});
    ...
    Tensor sdS = make_tensor(sQ.data() + size(sQ),typename Kernel_traits::SmemLayoutPdS{});
    ...
    typename Kernel_traits::GmemTiledCopyK_4x4 gmem_tiled_copy_K_4x4;
    ...
    int row = lane_id / 16;
    int col = lane_id % 16;
    int offset = warp_id * 16 + row * 4 * 32 + col % 4 * 32 + col / 4 * 4;
    int ldsQ_row = offset / 64;
    int ldsQ_col = offset % 64 / 4;
    ldsQ_col ^= (ldsQ_row % 16);
    Element *Qtsmem_ptr_lds = reinterpret_cast<Element *>(sQ.data().get()) + ldsQ_col * 4 + ldsQ_row * 64;
    Tensor tdKsQt = make_tensor(make_smem_ptr(Qtsmem_ptr_lds), make_layout(Shape<_4, _3, _1>{},
                                                                          Stride<_1, Int<kHeadDim/3*16>, _0>{}));
    Element *dOtsmem_ptr_lds = reinterpret_cast<Element *>(sdO.data().get()) + ldsQ_col * 4 + ldsQ_row * 64;
    Tensor tdVsdOt = make_tensor(make_smem_ptr(dOtsmem_ptr_lds), make_layout(Shape<_4, _3, _1>{},
                                                                          Stride<_1, Int<kHeadDim/3*16>, _0>{}));
    // // Copy V to shared , then to mtreg
    ...
    // sts pointer
    int ldsdSt_col = col ^ (row * 4);

    Element *dStsmem_ptr_sts = reinterpret_cast<Element *>(sdSt.data().get()) + warp_id * 4 * 64 + row * 64 + 4 * ldsdSt_col;
    Tensor tdStsdSt = make_tensor(make_smem_ptr(dStsmem_ptr_sts), make_layout(Shape<_4, _1, _2>{}, 
                                                                          Stride<_1, _0,Int<8*64>>{}));
    Element *Ptsmem_ptr_sts = reinterpret_cast<Element *>(sP.data().get()) + warp_id * 4 * 64 + row * 64 + 4 * ldsdSt_col;
    ...
    Element *dStsmem_ptr_lds = reinterpret_cast<Element *>(sdSt.data().get()) + lds_row * 64 + lds_col * 4;
    Tensor tdKsdSt = make_tensor(make_smem_ptr(dStsmem_ptr_lds), make_layout(Shape<_4, _4, _1>{},         // product = 16
                                                                          Stride<_1, Int<16*16>, _0>{}));
    Element *Ptsmem_ptr_lds = reinterpret_cast<Element *>(sP.data().get()) + lds_row * 64 + lds_col * 4; 
    Tensor tdVsPt = make_tensor(make_smem_ptr(Ptsmem_ptr_lds), make_layout(Shape<_4, _4, _1>{},         // product = 16
                                                                          Stride<_1, Int<16*16>, _0>{}));
    //////////////////////////////////////////////////////////////////////
    ...
    Layout tKrK_l = tKrK.layout();
    auto tKrK_view = make_tensor(tKrK.data(), make_layout(get<0, 0>(tKrK_l), get<0, 1>(tKrK_l), get<2>(tKrK_l)));
    permute_4x4_b16(tKrK_view);
    Element *Ktsmem_ptr_sts = reinterpret_cast<Element *>(sKt.data().get()) + tidx * 16;
    Tensor tKtsKt = make_tensor(make_smem_ptr(Ktsmem_ptr_sts), make_layout(Shape<_4, _4, _3>{},
                                                                          Stride<_1, _4, Int<32*64>>{}));

    Element *Ktsmem_ptr_lds = reinterpret_cast<Element *>(sKt.data().get()) + warp_id * 64 + row * 2 * 64 + col * 4;
    Tensor tdQsKt = make_tensor(make_smem_ptr(Ktsmem_ptr_lds), make_layout(Shape<_4, _3, _4>{},
                                                                             Stride<_1, Int<32*64>, Int<8*64>>{}));
    flash::sync_threads();
    cute::copy(tKrK_view, tKtsKt);
    flash::sync_threads();
    cute::copy(tdQsKt, tdQrKt);

    bool Is_first_loop = true;

    // Copy K to shared ,then to mtreg
    ...
    for (; m_block >= m_block_min; --m_block) {
        ...
        if (Is_first_loop) {// if first loop, stg Q and dO to save smem for sV
        ...
        flash::sync_threads();
        flash::lds_transpose(tdVsdOt, tdVrdO);
        flash::gemm<false, true>(acc_dv, tdVrPt, tdVrdO, tdVsPt, tdVsdOt, tiled_mma_dkv,
    ...
    Tensor sdV = make_tensor(sQ.data(), typename Kernel_traits::SmemLayoutdKV{}); // (SMEM_N, SMEM_K)
