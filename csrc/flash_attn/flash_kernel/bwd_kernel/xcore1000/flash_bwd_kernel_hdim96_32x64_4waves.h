...
inline __device__ void compute_dq_dk_dv_1colblock_hdim96_32x64_4waves(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    Tensor sKt = make_tensor(sK.data() + size(sK), typename Kernel_traits::SmemLayoutKtransposedNoSwizzle{});
    Tensor sKtNoSwizzle = make_tensor(sK.data() + size(sK), typename Kernel_traits::SmemLayoutKtransposedNoSwizzle{});
    ...
    Tensor sdS = make_tensor(sdO.data() + size(sdO),typename Kernel_traits::SmemLayoutPdS{});
    ...
    Tensor sV = make_tensor(sP.data() + size(sP), typename Kernel_traits::SmemLayoutKV{});
    ...
    // Copy KV to shared , then to mtreg
    ...
    int stsPdS_col = col ^ (row * 4);

    auto sp_ptr = sP.data().ptr_;

    auto stsPt_ptr0 = sp_ptr + warp_id * 4 * 64 + row * 64 + stsPdS_col * 4;
    auto stsPt_ptr1 = stsPt_ptr0 + 16 * 64;

    // lds pointer
    auto warp_group = warp_id % 2;
    ...
    auto ldsPt_ptr0 = sp_ptr + warp_group * 8 * 64 + lds_row * 64 + lds_col * 4;
    auto ldsPt_ptr1 = ldsPt_ptr0 + 16 * 64;
    auto ldsPt_ptr2 = ldsPt_ptr0 + 4 * 64;
    auto ldsPt_ptr3 = ldsPt_ptr2 + 16 * 64;

    /////////////////////////////////////////////////////////////////////////////////////////////////
    // dSt

    // sts pointer
    auto sdSt_ptr = sdSt.data().ptr_;

    auto stsdSt_ptr0 = sdSt_ptr + warp_id * 4 * 64 + row * 64 + stsPdS_col * 4;
    auto stsdSt_ptr1 = stsdSt_ptr0 + 16 * 64;

    // lds pointer
    auto ldsdSt_ptr0 = sdSt_ptr + warp_group * 8 * 64 + lds_row * 64 + lds_col * 4;
    auto ldsdSt_ptr1 = ldsdSt_ptr0 + 16 * 64;
    auto ldsdSt_ptr2 = ldsdSt_ptr0 + 4 * 64;
    auto ldsdSt_ptr3 = ldsdSt_ptr2 + 16 * 64;

    ///////////////////////////////////////////////////////////////////////////////////////////////////
    // dS

    // sts pointer
    auto sdS_ptr = sdS.data().ptr_;

    auto stsdS_ptr0 = sdS_ptr + warp_id * 4 * 64 + row * 64 + stsPdS_col * 4;
    auto stsdS_ptr1 = stsdS_ptr0 + 16 * 64;

    // lds pointer
    auto ldsdS_ptr0 = sdS_ptr + warp_group * 4 * 64 + row * 64 + stsPdS_col * 4;
    auto ldsdS_ptr1 = ldsdS_ptr0 + 8 * 64;
    auto ldsdS_ptr2 = ldsdS_ptr1 + 8 * 64;
    auto ldsdS_ptr3 = ldsdS_ptr2 + 8 * 64;
    /////////////////////////////////////////////////////////////////////////////////////////////////////

    cute::copy(gmem_tiled_copy_K_4x2, tKrK, tKsK);
    // cute::copy(gmem_tiled_copy_QKV, tVrV, tVsV);
    flash::sync_threads();
    ...
    Tensor tKrK_vew = make_tensor(tKrK.data(), make_layout(make_shape(size<0>(tKrK), size<1>(tKrK), size<2>(tKrK))));
    Element *Ktsmem_ptr_sts = reinterpret_cast<Element *>(sKt.data().get()) + tidx * 8;
    Tensor tKtsKt = make_tensor(make_smem_ptr(Ktsmem_ptr_sts), make_layout(Shape<_8, _1, _3>{},
                                                                          Stride<_1, _0, Int<32*64>>{}));

    Element *Ktsmem_ptr_lds = reinterpret_cast<Element *>(sKt.data().get()) + warp_id / 2 * 64 + row * 2 * 64 + col * 4;
    Tensor tdQsKt = make_tensor(make_smem_ptr(Ktsmem_ptr_lds), make_layout(Shape<_4, _3, _4>{},
                                                                             Stride<_1, Int<32*64>, Int<8*64>>{}));
    flash::sync_threads();
    cute::copy(tKrK_vew, tKtsKt);
    ...
    for (; m_block >= m_block_min; --m_block) {
        ...
        flash::sts_transpose(tPrP,stsPt_ptr0,stsPt_ptr1);
        ...
        *(reinterpret_cast<uint64_t *>(stsdS_ptr0)) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().get()));
        *(reinterpret_cast<uint64_t *>(stsdS_ptr1)) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().get() + 4));
        ...
        if(warp_id == 0 || warp_id == 1){
            *(reinterpret_cast<uint64_t *>(tdQrdS.data())) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().get()));
            *(reinterpret_cast<uint64_t *>(tdQrdS.data() + 4)) = *(reinterpret_cast<uint64_t *>(ldsdS_ptr1));
            cute::gemm(tiled_mma_dq, tdQrdS(_, _, 0), tdQrKt(_, _, 0), acc_dq);
            *(reinterpret_cast<uint64_t *>(tdQrdS.data() + 8)) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().get() + 4));
            cute::gemm(tiled_mma_dq, tdQrdS(_, _, 1), tdQrKt(_, _, 1), acc_dq);
            *(reinterpret_cast<uint64_t *>(tdQrdS.data() + 12)) = *(reinterpret_cast<uint64_t *>(ldsdS_ptr3));
            cute::gemm(tiled_mma_dq, tdQrdS(_, _, 2), tdQrKt(_, _, 2), acc_dq);
            cute::gemm(tiled_mma_dq, tdQrdS(_, _, 3), tdQrKt(_, _, 3), acc_dq);
        }else {
            *(reinterpret_cast<uint64_t *>(tdQrdS.data())) = *(reinterpret_cast<uint64_t *>(ldsdS_ptr0));
            *(reinterpret_cast<uint64_t *>(tdQrdS.data() + 4)) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().get()));
            cute::gemm(tiled_mma_dq, tdQrdS(_, _, 0), tdQrKt(_, _, 0), acc_dq);
            *(reinterpret_cast<uint64_t *>(tdQrdS.data() + 8)) = *(reinterpret_cast<uint64_t *>(ldsdS_ptr2));
            cute::gemm(tiled_mma_dq, tdQrdS(_, _, 1), tdQrKt(_, _, 1), acc_dq);
            *(reinterpret_cast<uint64_t *>(tdQrdS.data() + 12)) = *(reinterpret_cast<uint64_t *>(tdSrdS.data().get() + 4));
            cute::gemm(tiled_mma_dq, tdQrdS(_, _, 2), tdQrKt(_, _, 2), acc_dq);
            cute::gemm(tiled_mma_dq, tdQrdS(_, _, 3), tdQrKt(_, _, 3), acc_dq);
        }
        ...
        if (m_block > m_block_min) {
            ...
            flash::copy_b64<true, Is_even_K>(tdOgdO, tdOrdO, tQcQ, params.d);
        }

        load_A_warp2x2(tdVrPt,ldsPt_ptr0,ldsPt_ptr1,ldsPt_ptr2,ldsPt_ptr3);

        load_A_trans_warp2x2(tdKrdSt,ldsdSt_ptr0,ldsdSt_ptr1,ldsdSt_ptr2,ldsdSt_ptr3);

        flash::gemm<true,false>(acc_dv, tdVrPt, tdVrdO, tdVsPt, tdVsdOt, tiled_mma_dkv,
                    smem_tiled_copy_PdSt, smem_tiled_copy_QdOt, smem_thr_copy_PdSt, smem_thr_copy_QdOt);

        flash::gemm<true,false>(acc_dk, tdKrdSt, tdKrQt, tdKsdSt, tdKsQt, tiled_mma_dkv,
                    smem_tiled_copy_PdSt, smem_tiled_copy_QdOt, smem_thr_copy_PdSt, smem_thr_copy_QdOt);
    ...
    flash::copy_reg_to_global<Is_even_MN, Is_even_K>(
        tdKrdK, tdKgdK, tdKVcdKV, params.d, binfo.actual_seqlen_k - n_block * kBlockN
    );
    flash::copy_reg_to_global<Is_even_MN, Is_even_K>(
        tdVrdV, tdVgdV, tdKVcdKV, params.d, binfo.actual_seqlen_k - n_block * kBlockN
    );
}
