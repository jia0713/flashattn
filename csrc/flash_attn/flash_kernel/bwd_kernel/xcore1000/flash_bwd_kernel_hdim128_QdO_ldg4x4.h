...
__forceinline__ __device__ void compute_dq_dk_dv_1colblock_hdim128_32x64_QdO_ldg4x4(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    Tensor sQ = make_tensor(make_smem_ptr(reinterpret_cast<Element *>(smem_)), typename Kernel_traits::SmemLayoutQdOSwizzle{});
    Tensor sQt = make_tensor(sQ.data() + size(sQ), typename Kernel_traits::SmemLayoutQdOtSwizzle{});
    Tensor sQtNoSwizzle = make_tensor(sQt.data(), typename Kernel_traits::SmemLayoutQdOtransposedNoSwizzle{});
    Tensor sdO = make_tensor(sQt.data() + size(sQt), typename Kernel_traits::SmemLayoutQdOSwizzle{});
    Tensor sdOt = make_tensor(sdO.data() + size(sdO), typename Kernel_traits::SmemLayoutQdOtSwizzle{});
    Tensor sdOtNoSwizzle = make_tensor(sdOt.data(), typename Kernel_traits::SmemLayoutQdOtransposedNoSwizzle{});
    ...
    // sP and sdSt share the same memory
    Tensor sdSt = make_tensor(sP.data(), typename Kernel_traits::SmemLayoutPdS{});
    ...
    typename Kernel_traits::GmemTiledCopyQdO_D128 gmem_tiled_copy_QdO;
    ...
    Tensor tdVrdO = thr_mma_dkv.partition_fragment_B(sdOtNoSwizzle); // (MMA, MMA_K, MMA_N)
    ...
    Tensor tPsP = make_thr_tensor_stmatrix_trans(sP);      // ((Atom,AtomNum),PIPE_M,PIPE_N)
    ...
    Tensor tdStsdSt = make_thr_tensor_stmatrix_trans(sdSt);   // ((Atom,AtomNum),PIPE_M,PIPE_N)
    ...
    Tensor tQtsQt = make_thr_tensor_st_QdOt(sQt);
    Tensor tdOtsdOt = make_thr_tensor_st_QdOt(sdOt);

    Tensor tdKsQt = make_thr_tensor_ld_QdOt(sQt);
    Tensor tdVsdOt = make_thr_tensor_ld_QdOt(sdOt);
    ...
    clear(tVrV);
    flash::copy_4x4<Is_even_MN, Is_even_K>(
    ...
    clear(tKrK);
    flash::copy_4x4<Is_even_MN, Is_even_K>(
        gmem_tiled_copy_KV, tKgK, tKrK, tKVcKV, tKVpKV, binfo.actual_seqlen_k - n_block * kBlockN
    );
    ...
    clear(tdOrdO);
    flash::copy_4x4<Is_even_MN, Is_even_K>(
        gmem_tiled_copy_QdO, tdOgdO, tdOrdO, tQcQ, tQpQ, binfo.actual_seqlen_q - m_block * kBlockM
    );
    clear(tQrQ);
    flash::copy_4x4<Is_even_MN, Is_even_K>(
        gmem_tiled_copy_QdO, tQgQ, tQrQ, tQcQ, tQpQ, binfo.actual_seqlen_q - m_block * kBlockM
    );

    __syncthreads();
    cute::copy(smem_tiled_copy_QdO, tdOrdO, tdOsdO);
    cute::copy(smem_tiled_copy_QdO, tQrQ, tQsQ);
    permute_4x4_b16(tdOrdO(_, 0, 0));
    cute::copy(tdOrdO, tdOtsdOt);
    permute_4x4_b16(tQrQ(_, 0, 0));
    cute::copy(tQrQ, tQtsQt);
    ...
    for (; m_block >= m_block_min; --m_block) {
        ...
        for (int mi = 0; mi < size<0>(dS); ++mi) {
            ...
            for (int ni = 0; ni < size<1>(dS); ++ni) {
                dS(mi, ni) = pointwise_mult<Is_dropout>(scores(mi, ni), dS(mi, ni), dP_sum(mi));
            }
        }
        ...
        __syncthreads(); // Need syncthreads since we're writing to the same sdOt and sP location
        ...
        if (m_block > m_block_min) {
            ...
            flash::copy_4x4</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QdO, tdOgdO, tdOrdO, tQcQ, tQpQ);
            cute::copy(smem_tiled_copy_QdO, tdOrdO, tdOsdO);
            permute_4x4_b16(tdOrdO(_, 0, 0));
            cute::copy(tdOrdO, tdOtsdOt);
        }
        ...
        flash::gemm<true, true>(acc_dq, tdQrdS, tdQrKt, tdQsdS, tdQsKt, tiled_mma_dq,
        ...
        if (m_block > m_block_min) {
            ...
            flash::copy_4x4</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QdO, tQgQ, tQrQ, tQcQ, tQpQ);
            __syncthreads();
            cute::copy(smem_tiled_copy_QdO, tQrQ, tQsQ);
            permute_4x4_b16(tQrQ(_, 0, 0));
            cute::copy(tQrQ, tQtsQt);
