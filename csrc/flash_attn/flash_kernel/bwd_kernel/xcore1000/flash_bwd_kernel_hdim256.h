...
inline __device__ void compute_dq_dk_dv_1colblock_hdim256_32x32_8wave(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    constexpr int kHeadDimQ = Kernel_traits::VarHeadDim::kHeadDimQ;
    constexpr int kHeadDimV = Kernel_traits::VarHeadDim::kHeadDimV;
    constexpr int kBlockKSmemQ = Kernel_traits::VarHeadDim::kBlockKSmemQ;
    constexpr int kBlockKSmemV = Kernel_traits::VarHeadDim::kBlockKSmemV;
    constexpr int kNWarps = Kernel_traits::kNWarps;
    constexpr int kNThreads = Kernel_traits::kNThreads;
    constexpr int AtomLayoutMS = Kernel_traits::AtomLayoutMSdP;
    constexpr int AtomLayoutNS = Kernel_traits::AtomLayoutNSdP;
    constexpr int  MMA_M_SdP = kBlockM / AtomLayoutMS / 16;
    constexpr int  MMA_N_SdP = kBlockN / AtomLayoutNS / 16;
    constexpr int  MMA_K_SdP = kBlockKSmemQ / 16;
    constexpr int AtomLayoutMdQ = Kernel_traits::AtomLayoutMdQ;
    constexpr int AtomLayoutNdQ = kNWarps / AtomLayoutMdQ;
    constexpr int kBlockKGmemK = 128;
    constexpr int kBlockKSmemK = AtomLayoutNdQ == 8 ? 128 : kBlockKSmemQ;
    constexpr int  MMA_M_dQ = kBlockM / AtomLayoutMdQ / 16;
    constexpr int  MMA_N_dQ = kBlockKSmemK / AtomLayoutNdQ / 16;
    constexpr int  MMA_K_dQ = kBlockN / 16;
    constexpr int AtomLayoutMdKV = Kernel_traits::AtomLayoutMdKV;
    constexpr int AtomLayoutNdKV = kNWarps / AtomLayoutMdKV;
    constexpr int  MMA_M_dKV = kBlockN / AtomLayoutMdKV / 16;
    constexpr int  MMA_N_dKV = kBlockKSmemQ / AtomLayoutNdKV / 16;
    constexpr int  MMA_K_dKV = kBlockM / 16;
    constexpr bool Double_buffer = !Kernel_traits::No_double_buffer;
    constexpr bool attn_mask_elem_num_per_thread = kBlockM * kBlockN / kNThreads;
    const bool merge_attn_mask_ldg = attn_mask_elem_num_per_thread % 4 == 0 &&
                attn_mask_elem_num_per_thread > 0 && flash::use_attn_mask_merge_ldg(params);

    static_assert(kHeadDimQ % 64 == 0 && kHeadDimV % 64 == 0,
        "hdim should be multiple of 64.");
    ...
    Tensor sK = make_tensor(make_smem_ptr(reinterpret_cast<Element *>(smem_)), typename Kernel_traits::SmemLayoutKV_b128{});
    Tensor sKt = make_tensor(sK.data(), typename Kernel_traits::SmemLayoutKtransposedNoSwizzle{});
    Tensor sKtNoSwizzle = make_tensor(sK.data(), typename Kernel_traits::SmemLayoutKtransposedNoSwizzle{});

    Tensor sV = make_tensor(sK.data() + size(sK), typename Kernel_traits::SmemLayoutKV_b128{});
    ...
    Tensor sdO = make_tensor(sQ.data() +  size(sQ), typename Kernel_traits::SmemLayoutQdO_b128{});
    ...
    Tensor sdS = make_tensor(sdO.data() + size(sdO),typename Kernel_traits::VarHeadDim::SmemLayoutPdS{});
    Tensor sdSt = make_tensor(sdS.data() + size(sdS), typename Kernel_traits::VarHeadDim::SmemLayoutPdStransposed{});
    ...
    ////////////////////////////////////////////////////////////////////////////////////////
    // GmemTiledCopy, Global <-> Shared
    // QdO | KV      GmemTiledCopyQKV(dO)
    // dQ            GmemTiledCopydQ
    // dQaccum       GmemTiledCopydQaccumAtomAdd
    ////////////////////////////////////////////////////////////////////////////////////////
    typename Kernel_traits::GmemTiledCopyQdO_B128 gmem_tiled_copy_QdO;
    auto gmem_thr_copy_QdO = gmem_tiled_copy_QdO.get_thread_slice(tidx);
    typename Kernel_traits::GmemTiledCopyK_4x2 gmem_tiled_copy_K_4x2;
    auto gmem_thr_copy_K_4x2 = gmem_tiled_copy_K_4x2.get_thread_slice(tidx);
    ...
    Tensor tKgK = gmem_thr_copy_K_4x2.partition_S(gK);  // (KCPY, KCPY_N, KCPY_K)
    Tensor tKsK = gmem_thr_copy_K_4x2.partition_D(sK);
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////
    // TiledMma, generate the tensor in mtreg

    // TiledMmaSdP
    // S = Q.K , dp = dO.V
    typename Kernel_traits::TiledMmaSdP_b128 tiled_mma_sdp;
    // only wave 0-3 compute gemm S and gemm dP
    const int tidx_mma_sdp = tidx & 0xFF;
    auto thr_mma_sdp = tiled_mma_sdp.get_thread_slice(tidx_mma_sdp);
    ...
    // TiledMmadKV
    // dK = dSt.Qt , dV = Pt.dO
    typename Kernel_traits::TiledMmadKV_16x64x16 tiled_mma_dkv;
    ...
    Tensor tdVrdOt = thr_mma_dkv.partition_fragment_B(sdOtransposedNoSwizzle); // (MMA, MMA_K, MMA_N)

    Tensor acc_dk = partition_fragment_C(tiled_mma_dkv, Shape<Int<kBlockN>, Int<kHeadDimQ>>{});  // MMA, MMA_M, MMA_N
    Tensor acc_dv = partition_fragment_C(tiled_mma_dkv, Shape<Int<kBlockN>, Int<kHeadDimV>>{});  // MMA, MMA_M, MMA_N
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // SmemTiledCopy , generate form TiledMma

    // TiledMmaSdP, S = Q.K , dp = dO.V
    // A matrix, SmemTiledCopyQdO = make_tiled_copy_A(TiledMmaSdP)
    auto smem_tiled_copy_QdO = make_tiled_copy_A(typename Kernel_traits::UniversalCopyAtomB128{}, tiled_mma_sdp);
    auto smem_thr_copy_QdO = smem_tiled_copy_QdO.get_thread_slice(tidx_mma_sdp);
    Tensor tSsQ = smem_thr_copy_QdO.partition_S(sQ);
    Tensor tdPsdO = smem_thr_copy_QdO.partition_S(sdO);

    // B matrix, SmemTiledCopyKV = make_tiled_copy_B(TiledMmaSdP)
    auto smem_tiled_copy_KV = make_tiled_copy_B(typename Kernel_traits::UniversalCopyAtomB128{}, tiled_mma_sdp);
    auto smem_thr_copy_KV = smem_tiled_copy_KV.get_thread_slice(tidx_mma_sdp);
    ...
    // C matrix, SmemTiledCopyPdS = make_tiled_copy_C(TiledMmaSdP)
    auto smem_tiled_copy_PdS = make_tiled_copy_C(typename Kernel_traits::UniversalCopyAtomB64{}, tiled_mma_sdp);
    auto smem_thr_copy_PdS = smem_tiled_copy_PdS.get_thread_slice(tidx_mma_sdp);
    ...
    // TiledMmadKV,dK = dSt.Qt , dV = Pt.dO
    ...
    auto smem_tiled_copy_QdOt = make_tiled_copy_B(typename Kernel_traits::UniversalCopyAtomB64{}, tiled_mma_dkv);
    ...
    Tensor tKcK = gmem_thr_copy_K_4x2.partition_D(cK);
    ...
    if ((!Is_first && !Seq_parallel) || params.deterministic) { flash::sync_threads(); }
    ...
    flash::copy_multirow_b32<Is_even_MN, Is_even_K>(
        tKgK,tKrK,tKcK,params.d,binfo.actual_seqlen_k - n_block * kBlockN
    );
    flash::copy_b64<Is_even_MN, Is_even_K>(
        tVgV,tVrV,tVcV,params.d_value,binfo.actual_seqlen_k - n_block * kBlockN
    );
    cute::copy(gmem_tiled_copy_K_4x2, tKrK, tKsK);
    cute::copy(gmem_tiled_copy_V, tVrV, tVsV);
    flash::sync_threads();
    ...
    permute_4x2_b16(tKrK);
    Tensor tKrK_permute_view = make_tensor(tKrK.data(), make_layout(make_shape(size<0>(tKrK), size<1>(tKrK), size<2>(tKrK))));
    Element *Ktsmem_ptr_sts = reinterpret_cast<Element *>(sKt.data().get()) + tidx * 8;
    Tensor tKtsKt = make_tensor(make_smem_ptr(Ktsmem_ptr_sts), make_layout(Shape<_8, Int<size<1>(tKrK)>, Int<kHeadDimQ/kBlockKGmemK>>{},
                                                                          Stride<_1, Int<kBlockN*kBlockKSmemK/size<1>(tKrK)>, Int<kBlockN*kBlockKGmemK>>{}));

    Element *Ktsmem_ptr_lds = reinterpret_cast<Element *>(sKt.data().get()) + warp_id / AtomLayoutMdQ * 64 + row * 4 * 64 + col * 4;
    Tensor tdQsKt = make_tensor(make_smem_ptr(Ktsmem_ptr_lds), make_layout(Shape<_4, Shape<Int<MMA_N_dQ>, Int<kHeadDimQ/kBlockKSmemK>>, Int<MMA_K_dQ>>{},
                                                                            Stride<_1, Stride<Int<2*kBlockKSmemK>,Int<kBlockN*kBlockKSmemK>>, Int<16*kBlockKSmemK>>{}));
    flash::sync_threads();
    cute::copy(tKrK_permute_view, tKtsKt);
    flash::sync_threads();
    ...
    //////////////////////////////////////////////////////////////////////
    // P sts and lds
    // Pt(32,32) ,but sts_transpose to shape(16,64)
    int sts_col = col ^ (row * 4);
    Element *Ptsmem_ptr_sts = reinterpret_cast<Element *>(sP.data().get()) + warp_id % 4 * 4 * 64 + row * 64 + 4 * sts_col;
    Tensor tPtsPt = make_tensor(make_smem_ptr(Ptsmem_ptr_sts), make_layout(Shape<_4, _1, Int<MMA_N_SdP>>{},
                                                                          Stride<_1, _0, Int<16*16*kNWarps>>{}));
    Element *dStsmem_ptr_sts = reinterpret_cast<Element *>(sdSt.data().get()) + warp_id % 4 * 4 * 64 + row * 64 + 4 * sts_col;
    Tensor tdStsdSt = make_tensor(make_smem_ptr(dStsmem_ptr_sts), make_layout(Shape<_4, _1, Int<MMA_N_SdP>>{},
                                                                          Stride<_1, _0, Int<16*16*kNWarps>>{}));
    ...
    lds_col ^= (lds_row * 4);
    Element *Ptsmem_ptr_lds = reinterpret_cast<Element *>(sP.data().get()) + warp_id % AtomLayoutMdKV * 16 * 16 * MMA_K_dKV + lds_row * 64 + lds_col * 4;
    Tensor tdVsPt = make_tensor(make_smem_ptr(Ptsmem_ptr_lds), make_layout(Shape<_4, Int<MMA_M_dKV>, Int<MMA_K_dKV>>{},
                                                                          Stride<_1, Int<kBlockM*kBlockN/MMA_M_dKV>, Int<16*16>>{}));
    Element *dStsmem_ptr_lds = reinterpret_cast<Element *>(sdSt.data().get()) + warp_id % AtomLayoutMdKV * 16 * 16 * MMA_K_dKV + lds_row * 64 + lds_col * 4;
    Tensor tdKsdSt = make_tensor(make_smem_ptr(dStsmem_ptr_lds), make_layout(Shape<_4, Int<MMA_M_dKV>, Int<MMA_K_dKV>>{},
                                                                          Stride<_1, Int<kBlockM*kBlockN/MMA_M_dKV>, Int<16*16>>{}));
    ...
    flash::copy_b128<Is_even_MN, Is_even_K>(
        tdOgdO, tdOrdO, tdOcdO, params.d_value, binfo.actual_seqlen_q - m_block * kBlockM
    );
    ...
    for (; m_block >= m_block_min; --m_block) {
        flash::barrier<4>();
        cute::copy(gmem_tiled_copy_QdO, tQrQ, tQsQ);
        cute::copy(gmem_tiled_copy_QdO, tdOrdO, tdOsdO);
        ...
        if (warp_id < 4) {
            ...
            if constexpr (!Is_causal && !Is_local) {
                if constexpr (!Is_even_MN) {
                    if ((n_block + 1) * kBlockN >= binfo.actual_seqlen_k) {
                ...
                if (m_block * kBlockM < (n_block + 1) * kBlockN + binfo.actual_seqlen_q - binfo.actual_seqlen_k - params.window_size_right
                    ...
                    flash::apply_mask_local(scores, n_block * kBlockN + (tidx / 64 / AtomLayoutMS) * 16,
                                            ...
                                            params.window_size_left, params.window_size_right, AtomLayoutNS * 16);
                }
            }
            ...
            if constexpr (Is_dropout) {
                ...
                // one dropout block is 16x64,so we need remap n_block to dropout block idx
                int block_col_idx = n_block / (64 / kBlockN);
            ...
            if constexpr (Is_dropout) {flash::relu_(rP);}
        ...
        if (m_block > m_block_min) {
            ...
            flash::copy_b128<true, Is_even_K>(tdOgdO, tdOrdO, tdOcdO, params.d_value);
        ...
        clear(acc_dq);
        flash::sync_threads<4>();
        cute::copy(smem_tiled_copy_QdOt, tdKsQt, tdKrQt);
        Tensor tdKrQt_reshape = make_tensor(tdKrQt.data(), make_layout(make_shape(size<0, 0>(tdKrQt), size<0, 1>(tdKrQt), size<1>(tdKrQt)*size<2>(tdKrQt))));
        permute_4x4_b16(tdKrQt_reshape);
        flash::gemm<false, true>(acc_dk, tdKrdSt, tdKrQt, tdKsdSt, tdKsQt, tiled_mma_dkv,
                    smem_tiled_copy_PdSt, smem_tiled_copy_QdOt, smem_thr_copy_PdSt, smem_thr_copy_QdOt);

        flash::gemm<false,true>(acc_dq, tdQrdS, tdQrKt, tdQsdS, tdQsKt, tiled_mma_dq,
        ...
        Tensor tdVrdOt_reshape = make_tensor(tdVrdOt.data(), make_layout(make_shape(size<0, 0>(tdVrdOt), size<0, 1>(tdVrdOt), size<1>(tdVrdOt)*size<2>(tdVrdOt))));
        permute_4x4_b16(tdVrdOt_reshape);
        flash::gemm<false, true>(acc_dv, tdVrPt, tdVrdOt, tdVsPt, tdVsdOt, tiled_mma_dkv,
                    smem_tiled_copy_PdSt, smem_tiled_copy_QdOt, smem_thr_copy_PdSt, smem_thr_copy_QdOt);
    ...
    flash::reorder_C(acc_dk);
    flash::reorder_C(acc_dv);
    ...
    Tensor sdK = make_tensor(sQ.data(), typename Kernel_traits::SmemLayoutdKV{});  // (SMEM_N, SMEM_K)
    Tensor sdV = make_tensor(sQ.data() + size(sdK), typename Kernel_traits::SmemLayoutdKV{}); // (SMEM_N, SMEM_K)

    // Partition sdV and sdK to match the accumulator partitioning
    auto smem_tiled_copy_dK = make_tiled_copy_C(typename Kernel_traits::UniversalCopyAtomB64{}, tiled_mma_dkv);
    auto smem_tiled_copy_dV = make_tiled_copy_C(typename Kernel_traits::UniversalCopyAtomB64{}, tiled_mma_dkv);
    ...
    // If Is_last, there's already a flash::sync_threads() at the end of the loop.
    if constexpr (!Is_last) { flash::sync_threads(); }
