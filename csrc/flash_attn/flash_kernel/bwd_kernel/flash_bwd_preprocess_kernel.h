...
inline __device__ void dot_do_o(Tensor<Engine0, Layout0> const &do_, Tensor<Engine0, Layout0> const &o,
    ...
    // convert do and o to fp32
    CONVERT_TENSOR_TYPE(typename Engine0::value_type, float, do_reshaped, do_fp32)
    CONVERT_TENSOR_TYPE(typename Engine0::value_type, float, o_reshaped, o_fp32)
    #pragma unroll
...
inline __device__ void compute_dot_do_o(const Params &params) {
    ...
    const index_t row_offset_dq_accum = binfo.q_offset(dq_accum_batch_stride, dq_accum_head_stride, bidb)
        + (m_block * kBlockM + (params.cu_seqlens_q == nullptr ? 0 : 128ll * bidb)) * dq_accum_head_stride + bidh * params.d_rounded;
    const index_t row_offset_dpsum =
        (params.unpadded_lse ? (bidh * (params.total_q + maxValidBlockSizeM * params.b) +
                                binfo.q_offset(params.seqlen_q_rounded, 1, bidb) + maxValidBlockSizeM * bidb)
    ...
    dot_do_o<Kernel_traits::kGmemThreadsPerRowdO>(tdOrdO, tdOrO, dP_sum,
                                                kNthreads / (Kernel_traits::kGmemThreadsPerRowdO), params.p_dropout);
    if (Clear_dQaccum) {
