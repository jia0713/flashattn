...
void compute_params_numsplits(mcFlashAttn::Flash_fwd_params &params, const int num_splits, bool force_split_kernel){
    ...
    auto meta = select_fwd_split_meta(params);
    check_fwd_split_meta_supported(meta);
    set_fwd_split_meta(params, meta);

    const int num_n_blocks = (max_seqlen_k + meta.block_n - 1) / meta.block_n;
    const int num_m_blocks = (max_seqlen_q + meta.block_m - 1) / meta.block_m;

    if (p_dropout == 0.0f) {  // SplitKV is not implemented for dropout
        if (num_splits < 1) {
            const int AP_nums = dprops.multiProcessorCount;
            params.num_splits = num_splits_heuristic(batch_size * num_heads * num_m_blocks,  AP_nums * meta.block_num_per_ap,
                                                     num_n_blocks, 128);
        }
